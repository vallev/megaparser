...
class HttpUtil
{
    ...
    */
    ...
    public static function makeUrl($url, $default_scheme='http')
    {
        $p = parse_url($url);
        if (!isset($p['scheme']) && strpos($url, '//') === false) {
            $url = $default_scheme . '://' . $url;
            $p = parse_url($url);
        }

        $real_url = (isset($p['scheme'])?$p['scheme']:$default_scheme)."://" . $p['host'] . (isset($p['path'])?$p['path']:'/'). (isset($p['query'])?'?' . $p['query']:'');
